...
*/
static const char PROGMEM mainMenuStrings0[] = "Tremors & Steps";
static const char PROGMEM mainMenuStrings1[] = "Pulse & Oxygen";
static const char PROGMEM mainMenuStrings2[] = "REM Cycles";
static const char PROGMEM mainMenuStrings3[] = "Alarm";
static const char PROGMEM mainMenuStrings4[] = "Settings";
...
static const char PROGMEM alarmMenuStrings0[] = "Set Hour";
static const char PROGMEM alarmMenuStrings1[] = "Set Minute";
static const char PROGMEM alarmMenuStrings2[] = "Set Second";

static const char* const PROGMEM alarmMenuStrings[] = {
  alarmMenuStrings0,
  alarmMenuStrings1,
  alarmMenuStrings2,
};

const menu_info alarmMenuInfo = {
  3,
  alarmMenuStrings,
  alarmMenu,
};

const menu_info menuList[] = { mainMenuInfo, settingsMenuInfo, dateTimeMenuInfo, alarmMenuInfo };
#define mainMenuIndex 0
#define settingsMenuIndex 1
#define dateTimeMenuIndex 2
#define alarmMenuIndex 3
...
void saveAlarmCallback() {
  alarmData[alarmSelection] = alarmVariable;

  for(int i = 0; i < 3; i++){
    SerialMonitorInterface.println(alarmData[i]);
  }
}
...
void dateTimeMenu(uint8_t selection) {
  ...
  if (selection >= 0 && selection < 6) {
    ...
    if (selection == 0) {
      editInt(0, &dateTimeVariable, "Set Year", saveChangeCallback);
    } else if (selection == 1) {
      editInt(0, &dateTimeVariable, "Set Month", saveChangeCallback);
    } else if (selection == 2) {
      editInt(0, &dateTimeVariable, "Set Day", saveChangeCallback);
    } else if (selection == 3) {
      editInt(0, &dateTimeVariable, "Set Hour", saveChangeCallback);
    } else if (selection == 4) {
      editInt(0, &dateTimeVariable, "Set Minute", saveChangeCallback);
    } else if (selection == 5) {
      editInt(0, &dateTimeVariable, "Set Second", saveChangeCallback);
    }
  }
}
...
void settingsMenu(uint8_t selection) {
  ...
  if (selection == 1) {
    //char buffer[20];
    //strcpy_P(buffer, (PGM_P)pgm_read_word(&(menuList[settingsMenuIndex].strings[selection])));
    editInt(0, &sleepTimeout, "Auto-Off", NULL);
  }
  if (selection == 2) {
    //char buffer[20];
    //strcpy_P(buffer, (PGM_P)pgm_read_word(&(menuList[settingsMenuIndex].strings[selection])));
    editInt(0, &brightness, "Brightness", NULL);
  }
}

void alarmMenu(uint8_t selection) {
  if (menu_debug_print) SerialMonitorInterface.print("alarmMenu ");
  if (menu_debug_print) SerialMonitorInterface.println(selection);
  if (selection >= 0 && selection < 3) {
    alarmSelection = selection;
  }
  alarmVariable = alarmData[selection];
  alarmSelection = selection;
  if (selection == 0) {
    editInt(0, &alarmVariable, "Set Hour", saveAlarmCallback);
  } else if (selection == 1) {
    editInt(0, &alarmVariable, "Set Minute", saveAlarmCallback);
  } else if (selection == 2) {
    editInt(0, &alarmVariable, "Set Second", saveAlarmCallback);
  }
}
...
void viewMenu(uint8_t button) {
  ...
  if (lastMenuLine != currentMenuLine || lastSelectionLine != currentSelectionLine) {
    ...
    if (menu_debug_print) SerialMonitorInterface.println(currentMenu);
    for (int i = 0; i < 5; i++) {
      //SerialMonitorInterface.print("Drawing item: ");
      ...
      if (currentMenuLine + i < menuList[currentMenu].amtLines) {
        ...
        display.print(menuList[currentMenu].strings[currentMenuLine + i]);
      }
      for (uint8_t j = 0; j < 25; j++) display.write(' ');
      if (i == 0) {
        display.fontColor(defaultFontColor, inactiveFontBG);
        display.setCursor(0, menuTextY[0]);
        display.print('<');
    ...
    SerialMonitorInterface.println("Menu Drawn");
  }
}
